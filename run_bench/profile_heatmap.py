...
def merge_heats(filtered_sorted_all, abs_all):
    ...
    for index, latent_heat_row in filtered_sorted_all.iterrows():
        ...
        prev_index = index 
        next_index = index 
        while abs_all.loc[prev_index]['temp'] >= 0.1 and abs_all.loc[prev_index]['abs_time'] == cur_row_time:
        ...
        while abs_all.loc[next_index]['temp'] >= 0.1 and abs_all.loc[next_index]['abs_time'] == cur_row_time:
...
def write_res(to_DRAM_regions, file_path):
    with open(file_path, 'w') as file:
        for tuple_data in to_DRAM_regions:
...
if __name__ == "__main__":
    ...
    write_res(to_DRAM_regions, "/home/cc/res.txt")
